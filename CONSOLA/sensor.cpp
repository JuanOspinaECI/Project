...
#include "sensor.h"
	...
	Sensor Sensor::minimo(const Sensor &a, const Sensor &b){
		Sensor Sensor_TEMP;
        for(int i=0; i < _tamano; i++)
            if(a._dato[i] <= b._dato[i]){Sensor_TEMP._dato[i] = a._dato[i];}
			else{Sensor_TEMP._dato[i] = b._dato[i];}
...
/* GPS const GPS&::operator=(const GPS &a){
	...
	Temp_Hum TTemp_Hum operator+(const Temp_Hum &a){
	...
	Viento Viento::maximo(const Viento &a, const Viento &b){
		Viento Viento_TEMP;
        for(int i=0; i < _tamano; i++)
            if(a._dato[i] >= b._dato[i]){Temp_Hum_TEMP._dato[i] = a._dato[i];}
			else{Temp_Hum_TEMP._dato[i] = b._dato[i];}
	...
	Viento Viento::minimo(const Viento &a, const Viento &b){
		Viento Viento_TEMP;
        for(int i=0; i < _tamano; i++)
...
void Viento::actualizar(){
	...
	Pre Pre::maximo(const Pre &a, const Pre &b){
		Pre Pre_TEMP;
        for(int i=0; i < _tamano; i++)
	...
	Pre Pre::minimo(const Pre &a, const Pre &b){
		Pre Pre_TEMP;
        for(int i=0; i < _tamano; i++)
            if(a._dato[i] <= b._dato[i]){Pre_TEMP._dato[i] = a._dato[i];}
			else{Pre_TEMP._dato[i] = b._dato[i];}
