...
bool DB_Local::AbrirDB(){
    ...
    sqlstr << "CREATE TABLE IF NOT EXISTS TBL_Usuario (id_usuario INTEGER PRIMARY KEY NOT NULL," \
       ...
       "Documento TEXT NOT NULL, Fecha_nacimiento TEXT NOT NULL,"\
...
bool DB_Local::GuardarUser(std::string us, string nom, string ape, string doc, string fecha , string pass){
   char *zErrMsg = nullptr;
   stringstream sql;
   string hr, mn;
   hr = to_string(hora);
   mn = to_string(min);
   sql << "INSERT INTO TBL_Usuario(id_usuario, Nombre, Apellido, Documento, Fecha_nacimiento, user_name , passwd) VALUES (";
   sql << rand() << "', " << us <<"', ''"<< nom <<"'', ''" << ape << "', '" << doc << "', '" << fecha << "', '"<< us << "', '" << pass << "');";// << ");";
